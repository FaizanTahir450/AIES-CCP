...
l1 = [...] # Your original symptoms list
disease = [...] # Your original diseases list
...
def train_models():
    ...
    # Load training data
    df = pd.read_csv('flask project/dataset/Training.csv')
    tr = pd.read_csv('flask project/dataset/Testing.csv')
    
    # Data preprocessing (fix typos here)
    # ... your existing preprocessing code ...
    
    # Train models
    dt_model = DecisionTreeClassifier().fit(X, y)
    rf_model = RandomForestClassifier().fit(X, np.ravel(y))
    nb_model = GaussianNB().fit(X, np.ravel(y))
...
def index():
    if request.method == 'POST':
        ...
        # Make predictions
        dt_pred = disease[dt_model.predict([l2])[0]]
        rf_pred = disease[rf_model.predict([l2])[0]]
        nb_pred = disease[nb_model.predict([l2])[0]]
        
        return render_template('index.html', 
                             symptoms=l1,
                             results=[dt_pred, rf_pred, nb_pred])
...
def train_models():
    ...
    # Replace prognosis labels with numeric
...
def index():
    results = None
    if request.method == 'POST':
        # 1. grab the five submitted symptoms
        selected = {request.form.get(f'symptom{i}') for i in range(1, 6)}

        # 2. build a full-sized input vector
        l2 = [1 if symptom in selected else 0 for symptom in l1]
    ...
    return render_template('index.html', symptoms=l1, results=results)
